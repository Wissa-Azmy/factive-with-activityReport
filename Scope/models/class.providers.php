...
return  $embed;	
	...
	public function medielement($file,$thumb, $logo = null, $type=null) {
                      $embed = '<script src="' . site_url() . 'lib/players/media/mediaelement-and-player.min.js"></script> 
					  <script>$(\'#player1\').mediaelementplayer();</script>
					  ';
                      $embed .= '<link rel="stylesheet"  href="' . site_url() . 'lib/players/media/mediaelementplayer.css" />';
                      $embed .= '<video id="player1" height: "' . $this->height . '" width: "' . $this->width . '" poster="' . $thumb . '" controls="controls" preload="none">
					  <source type="video/' . str_replace("ogv", "ogg", $type) . '" src="' . $file . '"/>';
        ...
        <img src="' . $thumb . '" height: "' . $this->height . '" width: "' . $this->width . '" title="No video playback capabilities" />
          </object>
          </video>';
		  return  $embed;		
                  }
      public function remotevideo($url)
      {
		  ...
		  $pieces_array     = explode('.', $url);
				  ...
				  $embedCode = $this->medielement($url,thumb_fix($video->thumb),thumb_fix(get_option('player-logo')),$ext);
                  }	 
				  elseif ($choice == 5) {
      ...
      private function generateEmbedCode($videoProvider)
      {
          ...
          switch ($videoProvider) {
              ...
              case 'localfile':
				  ...
				  $embedCode = $this->medielement($real_link,thumb_fix($video->thumb),thumb_fix(get_option('player-logo')),$ext);
                  }	elseif ($choice == 5) {
