...
return site_url().page.url_split.nice_url($title).url_split.$id.'/';
}
function article_url($id, $title=null){
return site_url().blogpost.url_split.nice_url($title).url_split.$id.'/';
}
//Misc functions
...
if ($sub->nr > 999 && $sub->nr <= 999999) {
    $result = floor($sub->nr / 1000). _('k');
} elseif ($sub->nr > 999999) {
    $result = floor($sub->nr / 1000000). _('m');
} else {
    $result = $sub->nr;
}
return $result;
						...
						<div class="span4">
							<div class="innerT">
								<div class="text-center ">
									<a href="'.video_url($video->id , $video->title).'" title="'._html($video->title).'"><img src="'.thumb_fix($video->thumb, true, 200, 110).'" /></a>
								</div>
							</div>
						</div>
						<div class="span8">
...
if($video && $playlist) {
$did["what"] = _lang("collected a video in playlist ").' <a class="text-primary" href="'.playlist_url($playlist ->id , $playlist ->title).'" title="'._html($playlist ->title).'"><i class="icon-list-alt"></i> <strong>'._html(_cut($playlist ->title, 146)).'</strong></a>';
...
$video = $db->get_row("SELECT title,id,description, thumb  from ".DB_PREFIX."videos where id='".intval($done->object)."'");
if($video) {
$did["what"] = _lang("watched a video") .' <a class="text-primary" href="'.video_url($video->id , $video->title).'" title="'._html($video->title).'">'._html(_cut($video->title, 68)).'</a>';
...
$video = $db->get_row("SELECT title,id,description,thumb from ".DB_PREFIX."videos where id='".intval($done->object)."'");
if($video) {
$did["what"] = _lang("shared a video").' <a class="text-primary" href="'.video_url($video->id , $video->title).'" title="'._html($video->title).'">'._html(_cut($video->title, 68)).'</a>';
...
$did["what"] = _lang("liked a comment on the video") .' <a class="text-primary" href="'.video_url($video->id , $video->title).'" title="'._html($video->title).'">'._html(_cut($video->title, 68)).'</a>';;
if($video) {
$did["content"] = '<div class="content-filled">'.$com->comment_text.'</div>';
...
$list = token();
if($list) {
$videox = $db->get_row("select video_id as id from ".DB_PREFIX."playlist_data where playlist=$list  order by id desc");
if($videox){
return video_url($videox->id, 'playlist',$list );
}
}
...
return '<span>'._html(get_option('site-logo-text')).'</span>';
}
}
// duration to seconds
function _tSec($time) {
...
global $db;
if($type == 1) {
$ad = $db->get_row("select ad_content,ad_pos from ".DB_PREFIX."ads where ad_type = '1' ORDER BY rand()");
if($ad){
return '
...
$ad = $db->get_row("select ad_content from ".DB_PREFIX."ads where ad_type = '0' and ad_spot='".$spot."' ORDER BY rand()");
if($ad){
...
$data .= '<optgroup label="'.$cat["name"].'">';
	 $data .= ' <option value="'. $cat["id"].'">'. $cat["name"].'</option>';
	 if(isset($catarrays[$cat["id"]])) {
	 $data .=  rec_ch($catarrays[$cat["id"]], $catarrays['0']);
	 }
$data .= ' </optgroup>	';
} 
...
function rec_ch($in = array(), $full = array(), $pre = ''){
    $data = '';
	asort($in); 
	foreach ($in as $child) {
	$data .= ' <option value="'. $child["id"].'">'. $child["name"].'</option>';
	if(isset($full[$child["id"]])) {
	$data .= rec_ch($full[$child["id"]], $full);
	 }
	} 
return $data;
}
...
//MediaElement
function melementsup(){
   $jp = '<link rel="stylesheet"  href="' . site_url() . 'lib/players/media/mediaelementplayer.css" />';
   $jp .= ' <script src="' . site_url() . 'lib/players/media/mediaelement-and-player.min.js"></script>';
   return $jp;
                         }
//JwPlayer
 function jwplayersup(){
